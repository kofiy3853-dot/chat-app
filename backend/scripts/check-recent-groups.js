...
async function main() {
  ...
  try {
    ...
    const res = await client.query(`
      ...
      ORDER BY u.name, n."createdAt" DESC
    `);

    console.log('--- Last Hour Notifications ---');
    for (const row of res.rows) {
      ...
      console.log(`Recipient: ${row.recipientName}, Title: ${row.title}, Content: ${row.content}, Sender: ${senderName}, Profile: ${hasSenderProfile}, Type: ${row.type}, ID: ${row.id}`);
    }
