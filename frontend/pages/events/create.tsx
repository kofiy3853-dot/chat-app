      ...
      <main className="flex-1 p-6">
        <form onSubmit={handleSubmit} className="h-full flex flex-col">
          ...
          <footer className="mt-auto py-8 flex items-center space-x-4">
            ...
            {step < 5 ? (<button type="button" onClick={nextStep} disabled={!validateStep()} className="flex-[2] py-4 bg-primary-600 text-white rounded-[1.5rem] font-black text-xs uppercase shadow-lg disabled:opacity-50 flex items-center justify-center space-x-2"><span>Next Step</span><ChevronLeftIcon className="w-4 h-4 rotate-180 stroke-[3px]" /></button>) : (<button type="submit" disabled={loading} className="flex-[2] py-4 bg-emerald-600 text-white rounded-[1.5rem] font-black text-xs uppercase shadow-lg disabled:opacity-50 flex items-center justify-center space-x-2">{loading ? (<div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />) : (<><SparklesIcon className="w-4 h-4" /><span>Create Event</span></>)}</button>)}
          </footer>
        </form>
      </main>
    </div>
