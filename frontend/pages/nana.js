        ...
        <div className="max-w-5xl mx-auto w-full flex-1 flex flex-col">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 pb-6 border-b border-[#00ff41]/20">
            ...
            <div className="mt-4 md:mt-0 flex flex-wrap items-center gap-2 md:gap-4">
              ...
              <button onClick={() => router.push('/')} className="px-4 py-1 bg-blue-600/20 text-blue-400 border border-blue-500/30 rounded text-[10px] font-black uppercase hover:bg-blue-600/30">
                Access Inbox
              </button>
              <button 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
            ...
            <div className="lg:col-span-2 flex flex-col min-h-[500px]">
              <div className="bg-black/80 rounded-2xl border border-[#00ff41]/30 p-6 flex-1 flex flex-col shadow-2xl relative group">
                ...
                <div className="mt-6 pt-4 border-t border-[#00ff41]/10 flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
                  ...
                  <div className="flex-1 h-auto min-h-[36px] bg-[#00ff41]/5 rounded-lg px-3 py-2 flex flex-col sm:flex-row sm:items-center justify-between border border-[#00ff41]/20 gap-2">
                    ...
                    <button onClick={() => router.push('/')} className="px-4 py-1.5 bg-[#00ff41] text-black hover:bg-surface rounded text-[10px] font-black uppercase shrink-0 whitespace-nowrap">
                      Launch Communicator
                    </button>
                  </div>
                </div>
    ...
    <div className="flex flex-col h-screen bg-app relative overflow-hidden pt-14">
      ...
      >
        <div className="flex items-center space-x-4">
          <button onClick={() => router.push('/')} className="p-2 rounded-xl" style={{ backgroundColor: 'rgba(0,0,0,0.05)', color: 'var(--text-navbar)' }}>
            <ArrowLeftIcon className="w-5 h-5" />
          </button>
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setShowProfile(true)}>
            <div className="relative">
              <div className="w-8 h-8 rounded-full bg-surface flex items-center justify-center text-primary-600 font-black text-[12px] p-0.5 border border-[var(--divider)]">
              ...
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-[var(--bg-base)] shadow-sm flex items-center justify-center">
        ...
        <div className="flex items-center space-x-2">
          {conversationId && (
            <button onClick={handleClearChat} className="w-9 h-9 rounded-xl flex items-center justify-center border border-red-500/20 hover:bg-red-500/10 text-red-500" title="Clear Chat">
              <TrashIcon className="w-5 h-5" />
            </button>
          )}
          <button onClick={() => setShowTerminalOverlay(!showTerminalOverlay)} className="w-9 h-9 rounded-xl flex items-center justify-center border border-white/5" style={{ backgroundColor: 'rgba(0,0,0,0.05)', color: 'var(--text-navbar)' }}>
      ...
      <main className="flex-1 min-h-0 flex flex-col relative">
        <div className="flex-1 flex flex-col min-h-0">
          {loading ? (
            <div className="absolute inset-0 z-30 bg-app flex items-center justify-center">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              <div className="w-20 h-20 rounded-3xl bg-surface shadow-xl flex items-center justify-center mb-6">
                <SparklesIcon className="w-10 h-10 text-primary-400" />
              </div>
              <h3 className="text-lg font-black text-app-primary tracking-tight">Nana is unavailable</h3>
              <p className="text-xs text-app-muted font-bold mt-2 max-w-xs leading-relaxed uppercase tracking-widest">Unable to establish a secure connection to the AI Hub. Please check your internet and try again.</p>
              <button 
        ...
        {showTerminalOverlay && (
          <div className="absolute inset-x-4 bottom-24 bg-black/90 backdrop-blur-xl rounded-2xl border border-white/10 p-5 z-50 shadow-2xl shadow-black/40">
      ...
      {showProfile && (
        <>
          <div onClick={() => setShowProfile(false)} className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[99998]" />
          <div className="fixed inset-y-0 right-0 w-full max-w-sm bg-surface shadow-2xl z-[99999] flex flex-col border-l border-[var(--divider)]">
            <div className="flex items-center justify-between p-6 border-b border-[var(--divider)] bg-app/50">
              <h2 className="text-sm font-black text-app-primary uppercase tracking-widest">AI Hub Info</h2>
              <button onClick={() => setShowProfile(false)} className="p-2 bg-surface rounded-full text-app-muted hover:text-slate-600 shadow-sm border border-[var(--divider)]">
            ...
            <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center">
              ...
              <h1 className="text-2xl font-black text-app-primary tracking-tight text-center">Nana AI Agent</h1>
              ...
              <p className="text-sm font-bold uppercase tracking-widest mt-3 text-green-500"> Authorized & Secure </p>
            </div>
          </div>
        </>
      )}
